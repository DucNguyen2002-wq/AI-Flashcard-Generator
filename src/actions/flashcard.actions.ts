...
const flashcardSchema = z.object({
  ...
  question: z.string().min(1, "Câu hỏi không được để trống").max(500, "Câu hỏi tối đa 500 ký tự"),
  answer: z.string().min(1, "Câu trả lời không được để trống").max(1000, "Câu trả lời tối đa 1000 ký tự"),
})
...
export async function createFlashcard(formData: FormData) {
  ...
  if (!owned) {
    return { success: false, error: "Bộ thẻ không tồn tại hoặc bạn không có quyền truy cập" }
  }
  ...
  if (error) {
    return { success: false, error: "Không thể tạo flashcard. Vui lòng thử lại" }
  }
...
export async function updateFlashcard(id: string, formData: FormData) {
  ...
  if (error) {
    return { success: false, error: "Không thể cập nhật flashcard. Vui lòng thử lại" }
  }
...
export async function deleteFlashcard(id: string, deckId: string) {
  ...
  if (error) {
    return { success: false, error: "Không thể xoá flashcard. Vui lòng thử lại" }
  }
...
export async function saveGeneratedCards(
  ...
  const rows = cards.map((c) => ({ deck_id: deckId, question: c.question, answer: c.answer }))
  const { error } = await supabase.from("flashcards").insert(rows)

  if (error) {
    return { success: false, error: "Không thể lưu flashcard. Vui lòng thử lại" }
  }
...
export async function uploadDocument(formData: FormData) {
  ...
  if (file.type === "text/plain") {
    ...
    try {
      ...
      return { success: false, error: "Không thể đọc file PDF. Vui lòng thử lại" }
    }
  }
